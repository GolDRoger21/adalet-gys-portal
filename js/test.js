...
// VERİ & YAPILANDIRMA
// ===================================================================================
// YENİ TEST SORULARI İÇİN GOOGLE SHEET LİNKİ
const GOOGLE_SHEET_URL = "https://docs.google.com/spreadsheets/d/1syesXNc1pM1K-yZyd0saL4L4UYPjXpzmH1_SQwSXnYk/export?format=csv&gid=0";
...
class JusticeExamApp {
    ...
    initializeApp(questionPool) {
        ...
        this.domElements.startBtnFullText.textContent = `TESTE BAŞLA (${questionPool.length} Soru)`;
...
class ExamManager {
    ...
    finishQuiz(isAuto = false) {
        ...
        else { this.app.modalManager.show({ title: 'Testi Bitir', message: 'Testi bitirmek istediğinizden emin misiniz?', onConfirm: () => this.performFinish() }); }
    }
    ...
    handleVisibilityChange = () => {
        if (document.hidden) {
            this.app.modalManager.show({
                title: 'UYARI',
                message: 'Test sırasında başka bir sekmeye geçtiniz. Lütfen teste odaklanın.'
            });
        }
    }
    ...
    startTimer() {
        ...
        this.timerInterval = setInterval(() => {
            ...
            if (this.timeRemaining <= 300 && !this.app.domElements.remainingTime.classList.contains('timer-warning')) {
                ...
                this.app.domElements.timerAnnouncer.textContent = 'Testin bitmesine son 5 dakika kaldı.';
            }
        }, 1000);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchAndParseSheetData() {
        if (!GOOGLE_SHEET_URL) { app.showError("Google Sheet linki bulunamadı."); return; }
        try {
            ...
            if (rows.length < 2) throw new Error("CSV dosyasında yeterli veri bulunamadı (başlık ve en az bir soru satırı olmalı).");
            ...
            app.initializeApp(questionPool);
        } catch (error) { console.error('Veri çekme hatası:', error); app.showError(`Sorular çekilirken hata: ${error.message}`); }
    }
    fetchAndParseSheetData();
});
</script>
</body>
</html>
